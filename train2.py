...
ckpt_path = CKPT_DIR/"16x16/resol_16_iter_360000.pth"
...
while True:
    ...
    if step % 8000 == 0:
        ...
        save_image(
            grid, path=SAVE_DIR/f"""{phase}resol_{resol}_step{step}.jpg"""
        )

        save_parameters(
            model=gen,
            save_path=CKPT_DIR/f"""{phase}resol_{resol}_step{step}.pth"""
        )
